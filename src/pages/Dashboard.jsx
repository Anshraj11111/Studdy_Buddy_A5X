      ...
      <div className="relative flex-1 lg:ml-[240px] mt-16 px-3 sm:px-4 py-3 sm:py-5 space-y-3 sm:space-y-4 overflow-x-hidden" style={{ zIndex: 5 }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="relative rounded-lg overflow-hidden" style={{ minHeight: "260px", background: 'var(--bg-secondary)', border: "1px solid var(--border-primary)" }}>
              ...
              <div className="relative z-10 p-7">
                <motion.div className="flex items-center gap-2 mb-3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
                  ...
                  <span className="text-green-500" style={{ fontSize: "0.7rem", letterSpacing: "0.1em", textTransform: "uppercase" }}>System Online</span>
                  <span className="text-theme-tertiary" style={{ margin: "0 4px" }}>|</span>
                  <span className="text-theme-tertiary" style={{ fontSize: "0.7rem" }}>Studdy Buddy</span>
                </motion.div>
                <motion.div className="text-3xl font-bold text-theme-primary mb-1" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>Welcome back,</motion.div>
                <div className="flex items-center flex-wrap mb-3">
                  {(user?.name?.split(" ")[0] || "Student").split("").map((letter, i) => (
                    <motion.span key={i} className="text-3xl font-bold text-indigo-500"
                ...
                <motion.p className="text-theme-secondary" style={{ fontSize: "0.8rem", marginBottom: "1rem" }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.0 }}>
                  ...
                  <motion.span className="inline-block ml-1 align-middle" style={{ width: "2px", height: "14px", background: "#6366f1" }} animate={{ opacity: [1,0,1] }} transition={{ duration: 0.8, repeat: Infinity }} />
                </motion.p>
                ...
                <motion.div className="grid grid-cols-3 gap-3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.3 }}>
                  {[
                    { label: "My Doubts", sub: "Active", value: doubts.length, icon: MessageSquare, color: "#6366f1" },
                    ...
                    const Icon = s.icon
                    return (
                      <motion.div key={s.label} className="rounded-lg p-3" style={{ background: "var(--bg-primary)", border: "1px solid var(--border-primary)" }} whileHover={{ scale: 1.04 }}>
                        <div className="flex items-center gap-1.5 mb-1">
                          <Icon style={{ width: "0.85rem", height: "0.85rem", color: s.color }} />
                          <span className="text-theme-secondary" style={{ fontSize: "0.7rem" }}>{s.label}</span>
                        </div>
                        <div className="text-xl font-bold text-theme-primary">{s.value}</div>
                        <div className="text-theme-tertiary" style={{ fontSize: "0.65rem" }}>{s.sub}</div>
                      </motion.div>
                    )
                  })}
                </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
            <div className="rounded-lg p-5 h-full relative overflow-hidden" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-primary)", minHeight: "260px" }}>
              <div className="relative z-10">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <p className="text-indigo-500 mb-1" style={{ fontSize: "0.65rem", textTransform: "uppercase", letterSpacing: "0.1em" }}>// Level</p>
                    <motion.p className="text-2xl font-bold text-theme-primary" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>{xpLevel.level}</motion.p>
                    <motion.p className="text-2xl mt-1" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.7, type: "spring" }}>{xpLevel.icon}</motion.p>
                  </div>
                  <motion.div className="w-11 h-11 rounded-lg flex items-center justify-center bg-yellow-100 dark:bg-yellow-500/20" style={{ border: '1px solid #fcd34d' }}>
                    <Trophy style={{ width: "1.3rem", height: "1.3rem", color: "#f59e0b" }} />
                  </motion.div>
                </div>
                <div className="space-y-2 mb-4">
                  <div className="flex justify-between text-xs">
                    <span className="text-theme-tertiary">Progress</span>
                    <span className="text-indigo-500 font-medium">{xpLevel.progress}/{xpLevel.max} XP</span>
                  </div>
                  <div className="h-2 rounded-full overflow-hidden" style={{ background: "var(--bg-primary)" }}>
                    <motion.div initial={{ width: 0 }} animate={{ width: Math.min((xpLevel.progress / xpLevel.max) * 100, 100) + "%" }} transition={{ duration: 1.5, delay: 0.8, ease: "easeOut" }} className="h-full rounded-full" style={{ background: '#6366f1' }} />
                  </div>
                  <p className="text-theme-tertiary text-xs">{xpLevel.max - xpLevel.progress} XP remaining</p>
                </div>
                <div style={{ height: "1px", background: "var(--border-primary)", marginBottom: "12px" }} />
                <div className="flex items-center gap-2 text-theme-tertiary text-xs">
                  <Cpu style={{ width: "0.85rem", height: "0.85rem", color: "#6366f1" }} />
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <div className="flex items-center gap-2 mb-3">
            <div style={{ width: "2px", height: "18px", borderRadius: "99px", background: "#6366f1" }} />
            <h2 className="text-indigo-500 text-xs uppercase tracking-wider">// Quick Actions</h2>
          </div>
                ...
                <Link key={action.to} to={action.to} className="group">
                  <motion.div className="rounded-lg p-4 flex items-center justify-between" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-primary)" }} whileHover={{ y: -3, borderColor: action.color + "50" }} transition={{ duration: 0.2 }}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-theme-primary">{action.title}</p>
                        <p className="text-theme-tertiary text-xs">{action.desc}</p>
                      </div>
                    </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.8)", backdropFilter: "blur(8px)" }} onClick={() => setShowMatchModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="rounded-lg max-w-xl w-full max-h-[88vh] overflow-y-auto" style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-primary)" }} onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-5 space-y-4">
                <div>
                  <p className="text-indigo-500 text-xs uppercase tracking-wider mb-2">// Your Doubt</p>
                  <div className="rounded-lg p-4" style={{ background: "var(--bg-primary)", border: "1px solid var(--border-primary)" }}>
                    ...
                    <p className="text-theme-secondary text-xs mb-2">{selectedMatch.description}</p>
                    <Badge>{selectedMatch.topic}</Badge>
                  </div>
                </div>
                <div>
                  <p className="text-indigo-500 text-xs uppercase tracking-wider mb-2">// Matched Student</p>
                  {(() => {
                    ...
                    return (
                      <>
                        <div className="rounded-lg p-4 mb-3" style={{ background: "var(--bg-primary)", border: "1px solid var(--border-primary)" }}>
                          <div className="flex items-center gap-2.5 mb-3">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm" style={{ background: "#6366f1" }}>{matchedUser?.name?.charAt(0).toUpperCase()}</div>
                            <div>
                              <p className="font-semibold text-sm text-theme-primary">{matchedUser?.name}</p>
                              <p className="text-theme-tertiary text-xs">{matchedUser?.email}</p>
                            </div>
                          </div>
                          <h4 className="font-semibold text-sm text-theme-primary mb-1">{matchedDoubt?.title}</h4>
                          <p className="text-theme-secondary text-xs mb-2">{matchedDoubt?.description}</p>
